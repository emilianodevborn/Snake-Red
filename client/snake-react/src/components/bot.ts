...
export function computeBotState(gameState: GameState, snake: Snake): number[] {
  ...
  const obstacleFront = isObstacle(
    "front",
    gameState,
    snakeHead,
    snake.direction,
    ...
    ? 1
    : 0;
  const obstacleRight = isObstacle(
    "right",
    gameState,
    snakeHead,
    snake.direction,
...
export function mapActionToDirection(
  current: Direction,
  action: number[]
): Direction {
  ...
  let idx = directions.findIndex(
    (dir) => dir.x === current.x && dir.y === current.y
  );
  if (idx === -1) {
